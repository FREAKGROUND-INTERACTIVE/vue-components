<template>
  <div class="font">
    <template v-for="item in title" :key="item">
      <div class="font__phrase">
        <template v-for="char in item" :key="char">
          <div class="font__char">
            {{ char == " " ? "&nbsp;" : char }}
          </div>
        </template>
      </div>
...
export default {
  ...
  methods: {
    ...
    weightAnimation(elem, weight) {
      ...
      elem.style.fontSize = weight / 50 + 64 + "px";
